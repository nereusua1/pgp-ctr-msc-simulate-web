...
Create a GitHub issue in `nereusua1/pgp-ctr-msc-simulate-web`.

## When a skill says "fetch the relevant ticket"

Run `gh issue view <number> --comments` inside this repository.
...
- **Blocking**: prefer GitHub's native issue dependencies. Where dependencies are unavailable, use a `Blocked by: #<n>, #<n>` line at the top of the child body.
- **Frontier query**: list the map's open children, drop children with an open blocker or assignee, then take the first in map order.

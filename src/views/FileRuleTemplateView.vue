    ...
    <AppDetailPage v-else-if="canManage" :title="editorId === 'new' ? '新建文件规则模板' : `配置文件规则模板 · ${form.name}`" :before-close="confirmEditorClose" @close="closeEditor">
      ...
      <div class="editor-workbench">
        ...
        <section v-else id="file-rule-step-rules" class="config-panel"><div class="card-heading"><div><h2>时间替换规则</h2><p>从样例识别结果中选择文件名片段和内容字段，并为每条规则选择时间来源。</p></div></div>
          ...
          <div class="rule-trigger-guide"><b>时间来源说明</b><div><p v-for="source in FILE_TIME_SOURCES" :key="source[0]"><strong>{{ source[1] }}：</strong>{{ fileTimeSourceDescription(source[0]) }}</p></div><small>预报时间只适用于预报业务；实况的文件名和文件内容均只使用当前时间，<code>period_interval</code> 有值时向下对齐到最近间隔点，为空时不对齐。</small></div>

...
const evidenceSteps = computed(() => {
  ...
  const failure = filteredMessages.value.find(item => item.status === 'FAILED')?.failureStage || (selectedExecution.value.status === 'FAILED' ? selectedExecution.value.errorSummary : '')
  ...
  return stages.map((item, index) => ({ ...item, state: failedIndex === index ? 'failed' : (failedIndex >= 0 && index > failedIndex ? 'pending' : 'done') }))
})
...
watch(() => props.selectedExecutionId, executionId => {
...
<template>
  <main class="page execution-log-page">
    ...
    <section class="card execution-card">
      <div class="card-heading execution-heading">
        <div><h2>执行批次</h2><p>失败原因直接展示在列表中；点击详情可查看逐目标投递结果。</p></div>
        <div class="toolbar">
          <label class="toolbar-field"><span>搜索记录</span><SearchInput v-model="keyword" class="execution-search" aria-label="搜索执行日志" placeholder="搜索任务、报文、Execution 或目标" /></label>
      ...
      <div class="table-scroll">
        <table class="data-table execution-table">
          <thead><tr><th>执行批次</th><th>任务 / 报文</th><th>投递目标</th><th>执行结果</th><th>异常摘要</th><th>操作</th></tr></thead>
          <tbody>
            <tr v-for="item in filteredExecutions" :key="item.id" :class="{ 'failed-row': item.failed > 0 || item.status === 'FAILED' }">
              <td><button class="text-link execution-id" :disabled="isPending(`load-messages:${item.id}`)" @click="openExecution(item)">{{ item.id }}</button><small>{{ formatDateTime(item.startedAt) }}</small></td>
              <td><b>{{ item.taskName || '未知任务' }}</b><small>{{ item.messageName || '未关联报文' }}</small></td>
              <td><span class="target-text" :title="item.target">{{ item.target || '未进入 MQ 投递' }}</span></td>
              <td><StatusBadge :status="item.status" /><small>{{ item.success }} 成功 / {{ item.failed }} 失败</small></td>
              <td><span v-if="item.errorSummary" class="error-summary" :title="item.errorSummary">{{ item.errorSummary }}</span><span v-else class="muted">—</span></td>
              <td><button class="link-button" :disabled="isPending(`load-messages:${item.id}`)" @click="openExecution(item)">{{ isPending(`load-messages:${item.id}`) ? '正在加载…' : '查看详情' }}</button></td>
            </tr>
            <tr v-if="!filteredExecutions.length"><td colspan="6" class="empty-state">没有符合条件的执行记录。</td></tr>
          </tbody>
        </table>
      </div>
      <div class="pagination execution-pagination"><span>共 {{ executionPage.total }} 个执行批次，第 {{ executionPage.page }} / {{ executionPage.totalPages }} 页</span><div><button class="button secondary small" :disabled="executionPage.page <= 1 || isPending('query:executions')" @click="changePage(executionPage.page - 1)">上一页</button><button class="button secondary small" :disabled="executionPage.page >= executionPage.totalPages || isPending('query:executions')" @click="changePage(executionPage.page + 1)">下一页</button></div></div>
    </section>

    <AppModal v-if="selectedExecution" :title="detailMessage ? `最终生成报文 · ${detailMessage.id}` : '执行详情'" wide @close="closeExecution">
      ...
      <div v-if="detailMessage" class="message-detail-page unified-detail">
        ...
        <DetailGrid :columns="2"><div><dt>生成时间</dt><dd>{{ formatDateTime(detailMessage.generatedAt) }}</dd></div><div><dt>失败阶段</dt><dd>{{ detailMessage.failureStage || '无' }}</dd></div></DetailGrid>
      ...
      <div v-else class="execution-detail unified-detail">
        ...
        <DetailSection title="执行证据链" description="按真实执行结果标记完成节点和首个失败阶段">
          <section class="evidence-chain" aria-label="Execution 执行证据链"><div class="evidence-heading"><span>{{ selectedExecution.status === 'SUCCESS' ? '链路完整' : '存在中断' }}</span></div><ol><li v-for="step in evidenceSteps" :key="step.key" :class="step.state"><i></i><div><b>{{ step.label }}</b><small>{{ step.hint }}</small></div></li></ol></section>
        </DetailSection>
...
.detail-loading, .detail-error { display: flex; min-height: 84px; align-items: center; justify-content: center; gap: 12px; padding: 16px; border: 1px solid #dce4ef; border-radius: 11px; color: #667890; background: #fafbfd; font-size: 15px; }.detail-loading i { width: 17px; height: 17px; border: 2px solid #d9e3f2; border-top-color: #2d5fcf; border-radius: 50%; animation: detail-spin .8s linear infinite; }.detail-error { justify-content: space-between; border-color: #f0ccd0; background: #fff8f8; }.detail-error b { color: #a8414c; font-size: 15px; }.detail-error p { margin: 3px 0 0; color: #8d5a61; font-size: 13px; }
...
.summary-card { padding: 20px 22px; border: 1px solid var(--line); border-radius: 12px; background: #fff; }
...
.execution-table th, .execution-table td { vertical-align: middle; }
.execution-id { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; }
.data-table td small { display: block; margin-top: 6px; color: #8994a6; }
...
.evidence-chain { margin-top: 16px; padding: 17px 18px 18px; border: 1px solid #dfe6f0; border-radius: 12px; background: #fbfcfe; }
...
.execution-detail .evidence-chain { margin: 0; padding: 0; border: 0; background: transparent; }.execution-detail .evidence-heading { justify-content: flex-end; }.execution-detail .evidence-chain b { font-size: 14px; }.message-detail-page .code-block { max-height: 56vh; margin: 0; border-radius: 10px; font-size: 14px; line-height: 1.65; }

...
const serviceState = computed(() => loadError.value ? {label: '接口异常', tone: 'error'} : {
  label: '服务在线',
  tone: 'online'
})
...
async function queryExecutions(query = {}) {
  ...
  await runExclusive(pendingActions, 'query:executions', async () => {
    try {
      applyExecutionPage(await api.listExecutions(executionPage))
    } catch (error) {
      showError(error)
    }
  })
  ...
  <div v-else class="shell">
    <aside class="sidebar">
      ...
      <nav class="navigation" aria-label="主导航">
        <button v-for="item in navigation" :key="item.key" class="nav-item" :class="{ active: activePage === item.key }"
    ...
    <div class="content-shell">
      <header class="topbar">
        <div class="breadcrumb"><span>测试环境</span><i>/</i><b>{{ pageTitle }}</b></div>
      ...
      <main v-else-if="loadError" class="page">
        <section class="card empty-state"><h2>数据加载失败</h2>
          <p>{{ loadError }}</p>
          <button class="button primary" @click="loadAll">重新加载</button>
        </section>
      </main>
      <component v-else :is="currentComponent" v-bind="pageProps" @navigate="navigate" @task-action="openTaskDetail"

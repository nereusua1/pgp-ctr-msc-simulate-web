...
const drawer = await readFile(new URL('../src/components/AppDrawer.vue', import.meta.url), 'utf8')
...
test('报文编辑器采用全宽步骤工作台和无横向表格的映射式变量绑定', () => {
  assert.match(messageView, /class="editor-workbench"/)
  assert.match(messageView, /class="editor-step-header"/)
  assert.match(messageView, /<AppDrawer v-if="inspectorOpen" title="配置检查"/)
  ...
  assert.match(messageView, /基于当前未保存配置生成/)
  assert.doesNotMatch(messageView, /class="data-table time-binding-table"/)
  assert.doesNotMatch(messageView, /class="data-table value-binding-table"/)

...
test('真实页面组件可渲染两种角色与直接详情地址', async () => {
  ...
  try {
    ...
    assert.match(operator, /<section class="wide modal-card" role="dialog"/)
    assert.match(operator, /任务详情 · 气象补跑任务/)
    assert.match(operator, /task-list-card/)
    const admin = await renderToString(createSSRApp(Task, {...props, canManage:true}))
    assert.match(admin, /立即执行<\/button>/)
    const taskEditor = await renderToString(createSSRApp(Task, {...props, canManage:true, routeEdit:true}))
    assert.match(taskEditor, /配置任务 · 气象补跑任务/)
    assert.match(taskEditor, /保存修改/)
    assert.doesNotMatch(taskEditor, /任务详情 · 气象补跑任务/)
    assert.doesNotMatch(taskEditor, /task-list-card/)
    const taskCreator = await renderToString(createSSRApp(Task, {...props, canManage:true, routeId:'new', routeEdit:false}))
    assert.match(taskCreator, /新建任务/)
    assert.match(taskCreator, /detail-page/)
    assert.doesNotMatch(taskCreator, /modal-card/)
    assert.doesNotMatch(taskCreator, /task-list-card/)
    ...
    const {default: FileRule} = await server.ssrLoadModule('/src/views/FileRuleTemplateView.vue')
    const fileRuleList = await renderToString(createSSRApp(FileRule, {templates:[], canManage:true}))
    assert.match(fileRuleList, /新建文件规则模板/)
    assert.match(fileRuleList, /搜索文件规则模板/)
    assert.match(fileRuleList, /list-pagination/)
    assert.doesNotMatch(fileRuleList, /选择数据源/)
    const fileRuleEditor = await renderToString(createSSRApp(FileRule, {templates:[], canManage:true, routeId:'new'}))
    assert.match(fileRuleEditor, /保存后自动生成/)
    assert.match(fileRuleEditor, /样例与解析/)
    assert.match(fileRuleEditor, /三要素绑定/)
    assert.match(fileRuleEditor, /时间替换规则/)
    assert.match(fileRuleEditor, /detail-page/)
    assert.doesNotMatch(fileRuleEditor, /<label[^>]*>模板编码/)
    ...
    assert.match(overview, /关联模板尚未发布/)
